...
describe Statistics::ActivityPerCategoryType do
  ...
  context 'with transactions' do
    ...
    it 'returns an array with one element' do
      expect(subject.as_json.length).to eq 4
    end

    it 'returns the correct spent amount' do
      expect(subject.as_json[0]['spent']).to eq 30.0
      expect(subject.as_json[1]['spent']).to eq 40.0
      expect(subject.as_json[2]['spent']).to eq 0.0
      expect(subject.as_json[3]['spent']).to eq 70.0
    end

    it 'returns the correct received amount' do
      expect(subject.as_json[0]['received']).to eq 0.0
      expect(subject.as_json[1]['received']).to eq 0.0
      expect(subject.as_json[2]['received']).to eq 50.0
      expect(subject.as_json[3]['received']).to eq 0.0
    end

    it 'returns the correct net amount' do
      expect(subject.as_json[0]['net']).to eq(-30.0)
      expect(subject.as_json[1]['net']).to eq(-40.0)
      expect(subject.as_json[2]['net']).to eq(50.0)
      expect(subject.as_json[3]['net']).to eq(-70.0)
    end

    it 'returns the correct category type' do
      expect(subject.as_json[0]['type']).to eq "Essential"
      expect(subject.as_json[1]['type']).to eq "Flexible"
      expect(subject.as_json[2]['type']).to eq "Income"
      expect(subject.as_json[3]['type']).to eq "Uncategorised"
    end
  end
end
